...
class NoiseDataset(Dataset):
    ...
    def __init__(self, proton_filename: str, gamma_filename: str, max_samples: Optional[int] = None,
        ...
        original_proton_ratio = 0.5 # self.proton_metadata.num_rows / total_samples
        ...
        self._data = pd.concat([
            self.proton_data.assign(label=self.PROTON_LABEL),
            self.gamma_data.assign(label=self.GAMMA_LABEL),
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, int]:
        ...
        noisy_m1 = torch.tensor(row['noise_m1'], dtype=torch.float32)
        # clean_m1 = torch.tensor(row['clean_image_m1'], dtype=torch.float32)
        noisy_m2 = torch.tensor(row['noise_m2'], dtype=torch.float32)
...
def read_parquet_limit(filename: str, max_rows: int) -> pd.DataFrame:
    parquet_file_stream = pq.ParquetFile(filename).iter_batches(batch_size=max_rows)

    batch: pd.DataFrame = next(parquet_file_stream)
