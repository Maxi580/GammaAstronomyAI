...
def parameterize_HexCircleNetWithFeatures(trial: optuna.Trial):
    ...
    class HexCircleNet(nn.Module):
        def __init__(self):
            ...
            # FEATURE NET
            input_size = 51 # 51 features
            
            num_layers = trial.suggest_int('features_linear_layers', 1, 4)

            sizes = [input_size]
            for i in range(1, num_layers+1):
                sizes.append(trial.suggest_int(f'features_linear{i}_size', max(2, sizes[-1]//4), sizes[-1]*4))
            ...
            for i in range(num_layers):
                layers.extend([
                    nn.Linear(sizes[i], sizes[i + 1]),
                    nn.ReLU(),
                    nn.Dropout(trial.suggest_float(f'features_linear{i}_dropout', 0.05, 0.6))
                ])

            self.features_classifier = nn.Sequential(
                *layers,
                nn.Linear(sizes[-1], 2)
            )
        ...
        def forward(self, m1_image, m2_image, measurement_features):
            ...
            image_classified = self.classifier(combined)
            features_classified = self.features_classifier(measurement_features)

            return self.final_classifier(torch.cat([image_classified, features_classified], dim=1))
