...
and a graph-based Discriminator (with spectral normalization and LSGAN loss) that leverages
the hexagonal camera geometry. These modifications aim to stabilize the adversarial training.
"""
...
class GraphDiscriminator(nn.Module):
    def __init__(self, in_channels=1, hidden_channels=64, num_layers=3, seq_len=1039, k=6):
        ...
        The network outputs raw scores (not passed through sigmoid).
        """
        super().__init__()
...
def train():
    ...
    # Train the discriminator more times per generator update.
    disc_iters = 5
    ...
    for epoch in range(num_epochs):
        for batch_idx, (x_m1, x_c_m1) in enumerate(dataloader):
            ...
            # Prepare targets for discriminator:
            # - Background (mask==0): use true_noise as real (target=1)
            # - Particle regions (mask==1): use generated noise (detached) as fake (target=0)
            ...
            # ---------- Discriminator Training (multiple iterations) ----------
            ...
            # For generator: in particle regions push disc_pred_gen toward 1 (real)
