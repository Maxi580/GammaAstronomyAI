...
def exp_saturation(x, alpha=4.0, max_val=10.0):
    """
    Exponential saturation normalization:
      f(x) = max_val * (1 - exp(-x / alpha))
    This maps x in [0, ∞) to [0, max_val], saturating outliers.
    Assumes x is a torch.Tensor of nonnegative values.
    """
    return max_val * (1.0 - torch.exp(-x / alpha))
...
class Generator(nn.Module):
    def __init__(self, input_dim=2, d_model=256, nhead=4, num_layers=4, seq_len=1039):
        ...
        Generator takes a 2-channel input per pixel:
          - Channel 0: True noise (raw - cleaned) [normalized]
          - Channel 1: Binary mask (1 if cleaned pixel nonzero)
        and outputs a 1D noise prediction for each pixel.
        """
        super().__init__()
        self.input_linear = nn.Linear(input_dim, d_model)
        self.pos_enc = PositionalEncoding(d_model, dropout=0.05, max_len=seq_len)
...
# ------------------ Dataset with Global Normalization ------------------
class GammaDataset(Dataset):
    def __init__(self, parquet_file, seq_len=1039, alpha=1.0, max_val=10.0):
        self.df = pd.read_parquet(parquet_file)
        self.seq_len = seq_len
        # Store normalization parameters (global; these are fixed constants here)
        self.alpha = alpha
    ...
    def __getitem__(self, idx):
        ...
        # Load raw and cleaned images (first seq_len entries)
        x_m1 = torch.tensor(row["image_m1"][:self.seq_len], dtype=torch.float32)
        x_c_m1 = torch.tensor(row["clean_image_m1"][:self.seq_len], dtype=torch.float32)
        # ----- Apply Global Normalization -----
        # Using the exponential saturation function to map values to [0, max_val]
        x_m1 = exp_saturation(x_m1, alpha=self.alpha, max_val=self.max_val)
        x_c_m1 = exp_saturation(x_c_m1, alpha=self.alpha, max_val=self.max_val)
        # Note: any pixel that was exactly 0 remains 0, so the mask will be preserved.
        return x_m1, x_c_m1
...
def compute_distance_weights(mask, distance_matrix, d_max, pos_weight, neg_weight):
    ...
    for i in range(B):
        ...
        weights.append(w)
    plot_noise_comparison([weights[0]], ["Weights"])
...
# ------------------ Training Loop ------------------
def train():
    # Use the gamma file provided (adjust the filename as needed)
    gamma_file = "../magic-gammas.parquet"  # or adjust to your file path
    batch_size = 32
    num_epochs = 10
    lr = 1e-4
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # Create dataset with normalization parameters (alpha can be tuned)
    dataset = GammaDataset(gamma_file, seq_len=1039, alpha=1.0, max_val=10.0)
    ...
    # Base weights for loss computation
    pos_weight = 5.0  # For background (far from particle)
    neg_weight = 1.0  # For pixels on the particle region

    for epoch in range(num_epochs):
        for batch_idx, (x_m1, x_c_m1) in enumerate(dataloader):
            x_m1 = x_m1.to(device)  # (B, 1039), normalized raw image
            x_c_m1 = x_c_m1.to(device)  # (B, 1039), normalized cleaned image
            # Compute noise in the normalized domain.
            true_noise = x_m1 - x_c_m1  # (B, 1039)
            # The mask is computed on the cleaned image (0 remains 0)
            ...
            # Adversarial loss focused on particle regions (mask==1)
