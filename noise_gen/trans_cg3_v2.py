...
Discriminator (Transformer) is trained to distinguish “generated” noise (from particle regions)
from real noise (from background).
In this version, the Discriminator outputs probabilities in the range [0, 1],
where 1 indicates generated noise.
"""
...
class Generator(nn.Module):
    def __init__(self, input_dim=2, d_model=128, nhead=4, num_layers=2, seq_len=1024):
        """
...
class Discriminator(nn.Module):
    def __init__(self, input_dim=1, d_model=64, nhead=2, num_layers=2, seq_len=1024):
        """
        Discriminator takes a single-channel noise input per pixel (either real or generated)
        and outputs a probability per pixel indicating the likelihood that the noise
        is generated by the Generator.
        The output is in the range [0, 1], where 1 indicates generated noise.
        """
        super().__init__()
        self.input_linear = nn.Linear(input_dim, d_model)
        self.pos_enc = PositionalEncoding(d_model, dropout=0.1, max_len=seq_len)
    ...
    def forward(self, x):
        ...
        return torch.sigmoid(out).squeeze(-1)   # (batch, seq_len)
...
class GammaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Load a single row and extract the first 1024 values for each image.
        row = self.df.iloc[idx]
        x_m1 = torch.tensor(row["image_m1"][:1024], dtype=torch.float32)
        x_c_m1 = torch.tensor(row["clean_image_m1"][:1024], dtype=torch.float32)
        return x_m1, x_c_m1
...
def train():
    ...
    # Using BCELoss since discriminator outputs probabilities in [0,1]
    bce_loss = nn.BCELoss()
    l1_loss = nn.L1Loss()

    for epoch in range(num_epochs):
        for batch_idx, (x_m1, x_c_m1) in enumerate(dataloader):
            x_m1 = x_m1.to(device)           # (B, 1024)
            x_c_m1 = x_c_m1.to(device)         # (B, 1024)

            # Compute "true" noise = raw - cleaned.
            true_noise = x_m1 - x_c_m1         # (B, 1024)

            # Create binary mask: 1 for particle regions, 0 for background.
            mask = (x_c_m1 != 0).float()       # (B, 1024)

            # ------------------ Generator Forward Pass ------------------
            gen_input = torch.stack([true_noise, mask], dim=-1)  # (B, 1024, 2)
            gen_noise = generator(gen_input)                    # (B, 1024)
            ...
            disc_pred = discriminator(disc_input)  # (B, 1024)
            ...
            disc_pred_gen = discriminator(gen_noise)  # (B, 1024)
