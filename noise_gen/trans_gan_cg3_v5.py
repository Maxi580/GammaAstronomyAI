...
class GraphConvLayer(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        # edge_index[0] are target nodes; edge_index[1] are source nodes.
        row, col = edge_index
...
def train():
    ...
    # BCELoss with per-pixel weighting.
    ...
    for epoch in range(num_epochs):
        for batch_idx, (x_m1, x_c_m1) in enumerate(dataloader):
            ...
            # For discriminator:
            # - Background (mask==0): use true_noise → target 0.
            # - Particle region (mask==1): use gen_noise (detached) → target 1.
            ...
            # For generator: in particle regions we push disc_pred_gen toward 0.
            if mask.sum() > 0:
