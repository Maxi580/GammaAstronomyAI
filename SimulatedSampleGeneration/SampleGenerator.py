...
class SampleGenerator:
    def __init__(
        ...
        shapes: List[IShape],
        noise_generator: INoiseGenerator,
        image_dir: str,
        annotation_dir: str,
        array_dir: str,
        ...
        self.shape_probabilities = shape_probabilities or [1.0 / len(shapes)] * len(
            shapes
        )
    ...
    def generate_samples(self, num_samples: int):
        background, hexagons, plane_info = self.plane_generator.generate_plane(
            self.output_size, self.target_count
        )
        ...
        for i in range(1, num_samples + 1):
            ...
            shape = random.choices(self.shapes, weights=self.shape_probabilities, k=1)[
                0
            ]

            # Select random position for the shape
            shape_center_x, shape_center_y = shape_generator.get_random_position()
            shape_polygon = shape.generate_polygon(
                shape_center_x, shape_center_y, plane_info
            )
            ...
            for idx, (hex_points, hex_polygon) in enumerate(hex_polygons):
                if shape_polygon.intersects(hex_polygon):
                    ...
                    draw.polygon(
                        ...
                        outline=HexagonPlaneGenerator.HEXAGON_OUTLINE_COLOR,
                    )
