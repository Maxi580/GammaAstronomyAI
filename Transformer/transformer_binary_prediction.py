...
class ShapeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Target shape changed for BCEWithLogitsLoss
        y = torch.tensor(self.label_map[label_str], dtype=torch.float32).unsqueeze(0)
...
class ShapeTransformer(nn.Module):
    def __init__(self, emb_dim=128, n_heads=8, ff_dim=256, n_layers=4, max_len=2000):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(emb_dim, 1),  # Output a single value for binary classification
            # No Sigmoid here when using BCEWithLogitsLoss
        )
...
def evaluate_model(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for x, y in dataloader:
            ...
            # Threshold probabilities at 0.5 to get predictions
...
def report_misclassified(model, dataset, device, threshold=0.5):
    model.eval()
    count = 0
    with torch.no_grad():
        for i in range(len(dataset)):
            ...
            if pred != y.item():
                print(f"Sample {i} - Actual: {y.item()}, Predicted: {pred}, Probability: {prob}")
