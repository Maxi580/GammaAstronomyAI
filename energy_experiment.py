...
def train_with_energy_cutoff(model_name: str, proton_file: str, gamma_file: str,
                             min_true_energy: float, epochs: int):
    nametag = f"{model_name}_E{min_true_energy}_{time.strftime('%d-%m-%Y-%H-%M-%S')}"
    ...
    print(f"\t- True Energy Cutoff = {min_true_energy}")
    print(f"\t- Epochs = {epochs}")
    print(f"\t- Output = {output_dir}\n")

    dataset = MagicDataset(proton_file, gamma_file, debug_info=True, min_true_energy=min_true_energy)
...
def run_energy_cutoff_experiment(
    ...
    for cutoff in cutoffs:
        print(f"\n{'=' * 80}")
        print(f"Training with true energy cutoff: {cutoff}")
        print(f"{'=' * 80}\n")
...
def save_results(results, experiment_dir):
    results_file = os.path.join(experiment_dir, "true_energy_cutoff_results.json")
...
def plot_results(results, experiment_dir):
    ...
    fig.suptitle('Model Performance vs True Energy Cutoff', fontsize=16)
    ...
    axs[0, 0].set_xlabel('True Energy Cutoff')
    axs[0, 0].set_ylabel('Accuracy (%)')
    axs[0, 0].set_title('Accuracy vs True Energy Cutoff')
    axs[0, 0].grid(True, alpha=0.3)
    axs[0, 0].legend()

    axs[0, 1].plot(energy_cutoffs, val_precisions, 'g-o')
    axs[0, 1].set_xlabel('True Energy Cutoff')
    axs[0, 1].set_ylabel('Precision (%)')
    axs[0, 1].set_title('Validation Precision vs True Energy Cutoff')
    axs[0, 1].grid(True, alpha=0.3)

    axs[1, 0].plot(energy_cutoffs, val_recalls, 'm-o')
    axs[1, 0].set_xlabel('True Energy Cutoff')
    axs[1, 0].set_ylabel('Recall (%)')
    axs[1, 0].set_title('Validation Recall vs True Energy Cutoff')
    axs[1, 0].grid(True, alpha=0.3)

    axs[1, 1].plot(energy_cutoffs, val_f1s, 'y-o')
    axs[1, 1].set_xlabel('True Energy Cutoff')
    axs[1, 1].set_ylabel('F1 Score (%)')
    axs[1, 1].set_title('Validation F1 Score vs True Energy Cutoff')
    axs[1, 1].grid(True, alpha=0.3)

    plt.tight_layout(rect=(0, 0, 1, 0.96))

    plt_file = os.path.join(experiment_dir, "true_energy_cutoff_plots.png")
    ...
    plt.xlabel('True Energy Cutoff')
    plt.ylabel('Accuracy (%)')
    plt.title('Accuracy vs True Energy Cutoff')
    plt.grid(True, alpha=0.3)
    plt.legend()

    acc_plt_file = os.path.join(experiment_dir, "accuracy_vs_true_energy.png")
...
if __name__ == "__main__":
    ...
    print("Trained models at true energy cutoffs: {}".format([r['energy_cutoff'] for r in results]))