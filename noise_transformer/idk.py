...
class Generator(nn.Module):
    ...
    Generator takes a 2-channel input per pixel:
      ...
      - Channel 1: Binary mask (1 if cleaned pixel is nonzero)
    and outputs a 1D noise prediction per pixel.
    """
    ...
    def forward(self, x):
        x = self.input_linear(x)             # (B, 1039, d_model)
        x = self.pos_enc(x)
        x = x.transpose(0, 1)                # (1039, B, d_model)
        x = self.transformer(x)              # (1039, B, d_model)
        x = x.transpose(0, 1)                # (B, 1039, d_model)
        out = self.output_linear(x)          # (B, 1039, 1)
        return out.squeeze(-1)               # (B, 1039)

# ------------------ Transformer-based Discriminator ------------------
class TransformerDiscriminator(nn.Module):
    """
    Discriminator that uses a Transformer encoder to produce per-pixel predictions.
    It also precomputes a distance matrix from the MAGICCam geometry for use in
    loss weighting.
    """
    def __init__(self, input_dim=1, d_model=128, nhead=4, num_layers=2, seq_len=1039):
        super().__init__()
        self.input_linear = nn.Linear(input_dim, d_model)
        self.pos_enc = PositionalEncoding(d_model, dropout=0.1, max_len=seq_len)
        encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead)
        ...
        # Precompute distance matrix based on MAGICCam geometry.
        geom = CameraGeometry.from_name("MAGICCam")
        pix_x = geom.pix_x.value  # (num_pixels,)
        pix_y = geom.pix_y.value
        coords = np.stack([pix_x, pix_y], axis=1)  # (num_pixels, 2)
...
def compute_distance_weights(mask, distance_matrix, d_max, pos_weight, neg_weight):
    """
    Compute per-pixel weights based on distance to the nearest particle pixel.
    Pixels right on a particle (d==0) get neg_weight; pixels farther get pos_weight.
    """
    ...
    for i in range(B):
        ...
        if len(particle_idx) == 0:
            ...
            d_all = distance_matrix[:, particle_idx]  # (num_pixels, num_particle_pixels)
            d, _ = torch.min(d_all, dim=1)
...
def train():
    ...
    # Base weights for distance weighting.
    pos_weight = 5.0  # For pixels far from the particle.
    neg_weight = 1.0  # For pixels right on the particle.
    ...
    for epoch in range(num_epochs):
        for batch_idx, (x_m1, x_c_m1) in enumerate(dataloader):
            ...
            # Mask: 1 where the cleaned image is nonzero (particle), else 0.
            ...
            # For background (mask==0), use true noise; for particle regions, use generated noise.
            ...
            weights = compute_distance_weights(mask, discriminator.distance_matrix,
                                               discriminator.d_max, pos_weight, neg_weight)
            ...
            if batch_idx % 20 == 0:
                ...
                plot_noise_comparison(
                    [sample_true, sample_gen, mask[0], disc_pred[0], disc_input[0], masked_noise],
